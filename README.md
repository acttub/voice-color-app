...
1순위 지표는 **공유율**이다. 그 전엔 `analyze_start`(분석 시작)만 있고 **카드가 실제로 떴다는 이벤트가 없어** 분모를 만들 수 없었다.
...
| `analyze_fail` | 분석 실패 |
| `share_save` · `cta_click` | 이미지 저장 · acttub 이동 |
| `face_yes` · `face_no` · `face_skip` | 결과 화면의 확인 문항 응답 |

- 공유율 = `share_save` ÷ `result_view` · 완주율 = `result_view` ÷ `landing_view`
